...
// The overall process of baking a cake (preheat oven, mix ingredients, bake, and cool) is fixed,
...
2. Primitive Operations (Abstract Methods)
    The methods composeMessage() and sendMessage() are abstract, 
    meaning they must be implemented by subclasses (EmailNotification and SMSNotification) 
    to define specific behaviors for each notification type.
3. Concrete Operations (Final or Concrete Methods)Methods like rateLimitCheck, validateRecipient, preSendAuditLog, and postSendAnalytics are defined in the base class as concrete operations because they contain common logic shared by both email and SMS notifications.
    Methods like rateLimitCheck, validateRecipient, preSendAuditLog, 
    and postSendAnalytics are defined in the base class as concrete operations 
    because they contain common logic shared by both email and SMS notifications.
4. Hooks (Optional Methods with Default Behavior)
    The postSendAnalytics method is an optional hook 
    that can be overridden by subclasses 
    (e.g., SMSNotification overrides this method to provide custom analytics behavior). 
